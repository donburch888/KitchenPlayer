...
currPlaylist = ''
#currPlaylist = confparse.get("serverstats","lastsetpl")   ## the most currently loaded playlist.
...
#    if currPlaylist[:6] == "radio-":
    ...
    if proceed == messagebox.askokcancel("Are you sure ?",f"REMOVE {currSong['title']}" ):
        # remove from the playlist
        client.deleteid(songID)
        logger.info('LOG: removed {} by {} from playlist {}'.format(currSong['title'],currSong['artist'],currPlaylist) )
        client.save(currPlaylist)	#,replace)	# replace playlist file with modified version

        # delete the file from the music directory
        if os.path.isfile(path_to_dat / filename):
            logger.debug('remove    os.remove({})'.format( path_to_dat / filename) )
            os.remove(path_to_dat / filename)
        # log the file which was deleted (in case it should be reinstated manually later)
        logger.info('LOG: removed file {}'.format(filename) )
...
#    logger.debug("create button btnText={}, btnList({})={}".format(btnText,len(btnList),btnList) )
...
#    connext()
    ...
    if len(cpl) > 0:
        ...
        for plv in cpl:
            pl = plv['playlist'] + "," + pl
        #confparse.read(iniFilename)
        currPlaylist = confparse.get("serverstats","lastsetpl")
...
#    logger.debug(f"playlistType[{newPlaylist}]={playlistType[newPlaylist]}." )
    if playlistType[newPlaylist] == 'playlist':
        ...
        logger.info(f"Loadplaylist - unexpected playlistType '{playlistType[newPlaylist]}' for playlist '{newPlaylist}'")
    ...
    if 'error' in currStatus:
        ...
        logger.debug(f"MPD ERROR: {msg}.  playlist={currPlaylist}")
        messagebox.showinfo("MPD ERROR",msg)
        MPD('clear')
        currPlaylist = ""
        return				# don't action the error playlist

#    load artwork from playlistArt[newPlaylist]	# the URL of the image for the radio station
    ...
    updateIni("serverstats","lastsetpl",newPlaylist )
...
#
# Note: Just because this rogram has just started does not mean that 
...
if currStatus['state'] == 'play':
    currPlaylist = confparse.get("serverstats","lastsetpl")   ## the most recently loaded playlist.
    ...
    if 'songid' in currStatus:
        ...
        newPlaylist = confparse.get("serverstats","lastsetpl")   ## the most recently loaded playlist.
...
#        logger.debug(" ")
    ...
    #
    # if song has changed, update the Now playing information
    #
